...
app.post('/signup', async (req, res) => {
  ...
  try {
    const existingUser = await db.collection('Edutrack').where('email', '==', email).get();
    if (!existingUser.empty) {
      return res.send("User Already Exists!");
    }
    await db.collection('Edutrack').add({
...
app.post('/signin', async (req, res) => {
  ...
  try {
      ...
      const userRef = db.collection('Edutrack').where('email', '==', email).where('password', '==', password);
